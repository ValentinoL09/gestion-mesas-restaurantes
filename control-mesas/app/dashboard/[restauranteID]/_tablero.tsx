...
export default function Tablero({
  ...
  mesasIniciales: Tables<'mesas'>[];
  peticionesIniciales: Tables<'peticiones'>[];
}) {
  const [mesas, setMesas] = useState<Tables<'mesas'>[]>(mesasIniciales);
  const [peticiones, setPeticiones] = useState<Tables<'peticiones'>[]>(peticionesIniciales);
  ...
  useEffect(() => {
    ...
    // Magia en Vivo: escuchamos peticiones Y el estado de las mesas
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            try {
              new Audio('/alerta.mp3').play().catch(() => {});
            } catch {}
          }
          cargarDatos();
        }
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-xl border-l-4 shadow-sm bg-white ${
                      esCuenta ? 'border-green-500' : 'border-yellow-400'
                    }`}
                  >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {mesas.map((mesa) => {
              ...
              return (
                <div
                  key={mesa.id}
                  className={`relative flex flex-col items-center justify-center aspect-square rounded-2xl border-2 shadow-sm transition-all duration-300 ${colorFondo} ${animacion}`}
                >
